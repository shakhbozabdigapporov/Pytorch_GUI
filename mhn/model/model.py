...
class MHN():
	...
	def __init__(self, model_path, anchor_path='', conf_thres=0.25, iou_thres=0.5, visionin=True):
		self.visionin = visionin
	...
	def initialize_model(self, model_path, anchor_path, conf_thres=0.3, iou_thres=0.5):
	...
	def estimate_road(self, image):
		...
		# Process output data
		self.seg_map, self.filtered_boxes, self.filtered_scores, self.filtered_indexes = self.process_output(outputs)
	...
	def draw_boxes(self, image, text=True):
		...
		return util_draw_detections(self.filtered_boxes, self.filtered_scores, self.filtered_indexes, image, self.visionin, text)
